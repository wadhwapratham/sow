...
class VariantSelects extends HTMLElement {
  ...
  updateVariantTitle() {
    if (this.currentVariant) {
      var $this = this.currentVariant;
      var currentVariantId = this.currentVariant.id;
      var currentvarTitle = this.currentVariant.option1;
      $('.product__info-container').find('.selectedVarcolor').text(currentvarTitle);
      $('#productVariantTitle option').each(function() {
        var optionId = $(this).data('id');
        if(currentVariantId == optionId){
          var newTitle = $(this).text();
          $('.product__title h1').text(newTitle);
        }
      })
    }
  }
  ...
  updateTabContent() {
    if (this.currentVariant) {
      ...
      var newUrl = urlSplit + '=' + this.currentVariant.id;
      var $tabsection = $('#nutritionTabs');
      
      // get nutritional tabs data from url
      $.ajax({
        url : newUrl,
        type : "GET",
        success : function(response){
          var NutriContent = $(response).find('#nutritionTabs').html();
          $tabsection.html(NutriContent);
